...
def arg_parse():
    ...
    parser.add_argument('--attack_method', type=str, default='ImageGuidedFML2_Adam_MultiModels', help='')
    ...
    # 四个图像分类模型resnet, squeezenet, vgg, alexnet，每次选择一个，跑完image_main_ucf101.py ，再运行reference_ucf101文件。image_main_ucf101与reference_ucf101文件中的图像分类模型相对应。
    parser.add_argument('--direction_image_model', type=str, default='resnet', help='resnet, squeezenet, vgg, alexnet')
...
if __name__ == '__main__':
    ...
    nums_contained = int(101 / args.batch_nums)
    ...
    for step, data in enumerate(dataset_loader):
        if step >= left and step < right:
            ...
            if not os.path.exists(args.adv_path+args.direction_image_model):
                os.makedirs(args.adv_path+args.direction_image_model)
            for ind,label in enumerate(val_label):
                adv = adv_batches[ind].detach().cpu().numpy()
                np.save(os.path.join(args.adv_path+args.direction_image_model, '{}-adv'.format(label.item())), adv)
