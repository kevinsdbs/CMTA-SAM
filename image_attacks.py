...
class ImageGuidedFML2_Adam_MultiModels(Attack):
    '''
    The proposed Cross-modal Transfer Attack Method Based on Attention Mechanism (CMTA-SAM) attack.
    parameters:
        depth: {1,2,3,4}
    '''
    def __init__(self, model_name_lists, depths, epsilon=16/255, steps=60):
        ...
        self.attention_modules = {}  # 为每个模型初始化一个空间注意力模块。
        print(model_name_lists)
        for model_name in model_name_lists:
            self.attention_modules[model_name] = SpatialAttention()
    ...
    def forward(self, videos, labels, video_names):
        ...
        # adv_videos = videos.clone().detach().requires_grad_(True)
        # for step in range(self.steps):
        #     for model_name in self.model_name_lists:
        #         model = get_model(model_name)
        #         target_layer = self._find_target_layer(model, model_name)
        #         # 前向传播
        #         outputs = model(adv_videos)
        #         loss = F.cross_entropy(outputs, labels)
        #         # 计算梯度
        #         model.zero_grad()
        #         loss.backward(retain_graph=True)
        #         grad = adv_videos.grad.data
        #         # 应用空间注意力模块
        #         attention = self.attention_modules[model_name](adv_videos)
        #         grad = grad * attention
        #         # 更新对抗样本
        #         adv_videos = adv_videos.detach() + self.epsilon * torch.sign(grad)
        #         adv_videos = torch.clamp(adv_videos, 0, 1)
        #         adv_videos.requires_grad_(True)
        # return adv_videos
