...
import pandas as pd
import json
from transforms_ucf101 import computing_score
import torch
...
def arg_parse():
    ...
    parser.add_argument('--adv_path', type=str, default="Image-ImageGuidedFML2_Adam_MultiModels-10-", help='the path of adversarial examples.')
    parser.add_argument('--gpu', type=str, default='0', help='gpu device.')
    # 与image_main_ucf101文件中的图像分类模型相对应。
    ...
    if 'DATACENTER' in args.adv_path:
        ...
        args.adv_path = os.path.join(OPT_PATH, args.adv_path) + args.direction_image_model
    return args
...
def reference(model, files_batch, flag):
    ...
    with torch.no_grad():
        for step, batch in enumerate(files_batch):
            ...
            if step % 5 == 0:
                ...
                print(print_string)
                top = computing_score(model_name, top1.avg, flag)
...
if __name__ == '__main__':
    ...
    for model_name in MODEL_TO_CKPTS.keys():
        ...
        preds, labels, top1_avg = reference(model, files_batch, args.adv_path)
